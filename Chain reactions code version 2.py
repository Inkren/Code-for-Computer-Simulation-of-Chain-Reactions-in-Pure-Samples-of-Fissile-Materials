...
def fis(current):
    ...
    elif p0+p1<rando<=p0+p1+p2:
        for _ in range(2):
            newneutron= neu("l", simtime, current.pos)
            nel.insert(checker, newneutron)
            
    elif p0+p1+p2<rando<=p0+p1+p2+p3:
        for _ in range(3):
            newneutron= neu("l", simtime, current.pos)
            nel.insert(checker, newneutron)
            
    elif p0+p1+p2+p3<rando<=p0+p1+p2+p3+p4:
        for _ in range(4):
            newneutron= neu("l", simtime, current.pos)
            nel.insert(checker, newneutron)
            
    elif p0+p1+p2+p3+p4<rando<=p0+p1+p2+p3+p4+p5:
        for _ in range(5):
            newneutron= neu("l", simtime, current.pos)
            nel.insert(checker, newneutron)
            
    elif p0+p1+p2+p3+p4+p5<rando<=p0+p1+p2+p3+p4+p5+p6:
        for _ in range(6):
            newneutron= neu("l", simtime, current.pos)
            nel.insert(checker, newneutron)
...
while len(nel) > 0 and len(nel) < 500:
    ...
    while current.lif=="l":
        ...
        neutronradius=radius(current.pos) #Checks whether the neutron is in the limit of the simulation
        if neutronradius>lim:
            orp(current)
        else:
...
#Add tamper options
#Adjust mean free paths and time look at pg431 and onward of "An Introduction to Computer Simulation Methods Third Edition" equation 11 63
