...
tamper= input("What material do you want for the tamper if any?: No tamper, Depleted Uranium 238 [no/ur]:")
#page 71 of physics of manhatten project for critical mass for infinite tamper
#page 120 has an interesting idea for capture chance

lim= float(input("What is the radius of the sample in cm?:")) 
"Calculates total volume and mass of the sample"
volume=(4/3)*np.pi*(lim)**3 #need volume to get total mass
mass= volume*density

if tamper=="ur":
    tampmat="Uranium-238"
    tampermfp=4.342 #tamper constants from physics of manhatten project pg 193
    tamperpfis=0.308/7.773
    tamperpabs=0.066/7.773
    tamperpelastic=4.804/7.773
    tamperpinelastic=2.595/7.773
    tamperdensity=18.95
    tamperlim=float(input("How thick should the tamper be in cm?:"))

"calculates tamper volume and mass"
if tamper != "no":
    tampervolume=((4/3)*np.pi*(lim+tamperlim)**3)-volume
    tampermass=tampervolume*tamperdensity
#Starting values

simtime=0

velocity=1956000000#Velocity of a fast neutron past 2MeV from https://pubs.aip.org/aapt/ajp/article/82/10/972/1039414/fissioncore-A-desktop-computer-simulation-of-a

"Neutron values"
class neu:
    def __init__(self, lif, starttime, pos, tampermode):
        ...
        self.tampermode=(tampermode)#saves whether the neutron has inelastically scattered in the tamper
...
time={0: 1} #Stores total time steps
...
def tamperinteraction1(current):
    p=np.random.uniform(0, 1) #Randomises a number and sees which interaction happens
    if p<=tamperpfis:
        fission(current)
    elif p<=tamperpfis+tamperpabs:
        absorb(current)
    elif p<=tamperpfis+tamperpabs+tamperpelastic:
        scatter(current)
    else:
        current.tampermode=2 #Marks if the neutron has inelastically scattered in the tamper
        scatter(current)
        
def tamperinteraction2(current): #Interaction for neutrons that have inelastically scattered in the tamper
    p=np.random.uniform(0, 1) #Randomises a number and sees which interaction happens
    if p<=tamperpfis:
        fission(current)
    elif p<=tamperpfis+tamperpabs:
        ...
        absorb(current)#If the neutron inelastically scatters in the tamper again it is absorbed
...
def fission(current):
    ...
    if rando<=p0:
        for _ in range(0):
            newneutron= neu("l", simtime, current.pos, 1)
...
for i in range(10):
    nel.append(neu("l", 0, (0, 0, 0), 1))#Adds 10 neutrons to the list at the start 
timelim = 0
counter=0
simdone = False
while len(nel) > 0 and not simdone:
    ...
    while current.lif=="l":
        if tamper=="no":
            ...
            neutronintamper=radius(current.pos)
            if neutronintamper>lim:
                d=distance(tampermfp)
            else:
                d=distance(mfp)
        ...
        if sampleradius>lim:#this checks if the neutron is out of the radius
            if tamper=="no":
                if sampleradius> lim:#This is the effective radius. Double check source library.institutpendidikan.ac.id/wp-content/uploads/2024/08/Stacey-Nuclear_Reactor_Physics-2nd.pdf page 47 eqn 3.15
                    absorb(current)
                else:
                    interaction(current)
            else: #
                if sampleradius>lim+tamperlim:
                    absorb(current)
                else:
                    if current.tampermode==1:
                        tamperinteraction1(current)
                    else:
                        tamperinteraction2(current)
        else:
...
if tamper=="no":#checks if the graph is for a simulation with or without a tamper
    plt.figure()
    ...
    plt.xlabel("Time(nanoseconds)")
    plt.ylabel("live nuetrons")
    plt.title(f"Live neutrons over time for radius = {lim} cm in {mat} \n with tamper of {tamperlim} cm thick {tampmat}")
    plt.grid()
    plt.show()

print("Mass of radioactive core =", {mass},  "grams")
if tamper!="no":
    print("Mass of tamper =", {tampermass}, "grams")      
